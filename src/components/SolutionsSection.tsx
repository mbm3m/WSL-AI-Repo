...
const SolutionsSection = () => {
  ...
  return <section ref={sectionRef} className={`py-32 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} transition-opacity duration-500 opacity-0`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
        <div className="flex flex-col md:flex-row md:items-center gap-16">
          <div ref={contentRef} className="md:w-1/2 transition-opacity duration-500 opacity-0 delay-150">
            <h2 className={`text-4xl md:text-5xl font-display font-bold tracking-tight ${theme === 'dark' ? 'text-white' : 'text-gray-900'} mb-6`}>
              How Our Platform Solves It
            </h2>
            
            <p className={`text-xl ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'} mb-12 font-light`}>
            ...
            <ul className="space-y-8">
              ...
              return <li key={index} className="feature-item flex items-start transition-opacity duration-300 opacity-0">
                    ...
                    <div>
                      <h3 className={`text-lg font-display font-medium ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{feature.title}</h3>
                      <p className={`mt-1 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'} font-light`}>{feature.description}</p>
                    </div>
                  </li>;
            })}
            </ul>
          ...
          <div ref={imageRef} className="md:w-1/2 mt-12 md:mt-0 transition-opacity duration-500 opacity-0 delay-300">
            <div className="relative">
              ...
              <img alt="Medical staff using laptop" className="relative w-full rounded-2xl shadow-xl" src="/lovable-uploads/3fc1f935-c2ae-4dd2-8ff4-042798f22e02.png" />
            </div>
          </div>
        </div>
      </div>
